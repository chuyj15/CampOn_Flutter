...
class ProductReviewUpdate extends StatefulWidget {
  final int reviewNo;
  const ProductReviewUpdate({super.key, required this.reviewNo});

  @override
  State<ProductReviewUpdate> createState() => _ProductReviewUpdateState();
}

class _ProductReviewUpdateState extends State<ProductReviewUpdate> {
  ...
  Board board = Board();
  int reviewNo = 29; // 초기화 값 0 으로 바꿔야함
  @override
  void initState() {
    super.initState();
  ...
  Future _reviewRead() async {
    print('ReviewRead...');
    final url = Uri.parse("http://10.0.2.2:8081/api/board/prread/$reviewNo");
